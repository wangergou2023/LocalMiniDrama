...
1. **动作单元划分**：每个分镜 = **一次连续拍摄**，对应剧本中的一个叙事节拍。
   **默认一个分镜只做一件事；镜内切镜是例外，不是常态。**
   ...
   - **同一空间、同一时间线内的连续动作尽量一镜到底**（可以含 3-5 拍：起势→过程→收尾），**不要按拍拆镜**
   - **只有**空间／主体／时间发生切换，或必须靠剪辑才能表达的叙事跳跃，才拆新分镜；其余一律用「给足时长」解决
   - **判断标准（重要）**：分镜 = **一次连续拍摄**。内容演不完**不要拆镜**，而是**把这一镜的 duration 给足**（上限 = 项目配置的「每段最大秒数」）；
     只有**场景、主体或时间发生切换**，或者出现了必须靠剪辑才能表达的叙事跳跃时，才拆出新分镜
   - **要点不得丢（硬性，与「不要拆镜」并行）**：剧本里这一幕（或这一段）的画面描述常常含**多个视觉要点**
     （多句，或用分号／句号并列）。必须按**先后顺序**把这些要点分配到这一幕下的各条分镜，
     **一个要点都不许丢**：
     · 尤其不许漏掉**具体道具与量值** —— 型号、毫米、毫安时（mAh）、毫瓦（mW）、容量、时间、数量；
     · 宁可一条分镜只讲一个要点，也不要把几个要点概括成一句含糊的话；
     · 分镜条数要与该幕的要点数匹配：要点多就多拆一条，不要硬把多个要点塞进同一条
...
【重要提示】
- 镜头数量由内容决定，**不要为了凑数把一次连续拍摄切成多镜**：单镜 duration **常规 8-15 秒**（下限 5 秒、上限＝项目配置的「每段最大秒数」，见下方时长说明）；
  **含对白的镜头必须满足 duration ≥ 台词字数 ÷ 4.2 + 1 秒**（中文语速约 4.2 字/秒）—— 否则台词根本说不完，这也是「碎镜」最常见的成因；
...
- **每镜必写清单**：① 景别与机位；② 镜头怎么动（**按叙事需要，不是必须动**）；③ 主体动作与**表演**（微表情/视线/呼吸/身体反应，见下条"表演必须写出来"）；④ 环境与光照；⑤ 音效；⑥ 有台词则写对白。
...
- **运镜必须带动机（最重要的一条）**：写运镜不是写名称，要写清**方向 + 速度 + 跟随对象 + 叙事目的**。
  常用动机对照（说不出动机就用固定机位）：
    固定镜头＝让表演主导、克制观察（**默认选择**）；慢推＝情绪加压、逼近真相；急推＝冲击/惊讶；
    拉远＝孤立主体、揭示环境、情绪抽离；横摇/竖摇＝跟随视线、揭示画外信息或高度身份；
    跟拍＝沉浸、速度、行动连续（跟随前进/后退/穿越空间）；环绕＝**关系逆转、情绪旋涡（只在转折点用，不要每镜都绕）**；
    升降＝权力变化、空间揭示；手持微晃＝紧张临场；甩镜＝快速转移注意力。
  好写法：「中近景微推，镜头从她平静的肩线缓慢推进到克制的眼神，突出面对质问时的压迫感」；
  坏写法：「电影感运镜」「高级镜头」—— 只写名称或形容词等于没写。
- **运镜反面清单（出现即不合格）**：① 每镜都推近/都环绕，节奏单调；② 运镜只写名称，没有方向/速度/对象/动机；
...
- **对白镜禁止切拍（硬规则）**：有台词的镜头**一律单镜一镜到底**，不得使用镜内剪辑点 ——
  切镜会把一句台词拦腰截断，口型与情绪直接断掉（实测《焚毁纺锤》：44 字台词被 4 拍结构切成两段、用 <scenetrans> 跨拍）。
  做法：整句台词完整落在同一拍里；镜头要动就压在台词**之前**，台词期间锁死。
  **只有当单句台词所需秒数（字数 ÷ 4.2 + 1）超过单镜上限（15 秒）时**，才允许跨拍，且**两侧都必须写 <scenetrans>**。
...
- **表演必须写出来（硬规则，治"木讷"）**：每镜至少写 **2-3 个可演的微表演细节**，而且要写出**变化**（起 → 变），
  不能只写「站着 / 看着 / 含笑 / 神情复杂」这类状态词。可写的细节：**视线落点与转移、眉心与眼睑、嘴角与下颌、
  呼吸与胸口起伏、喉结吞咽、手指与指节、肩线与重心、身体朝向（前倾/后仰/后退）**。
  例：「她先是垂着眼、指尖抠着衣角（起），听到那句话后睫毛一颤、指节骤然收紧、肩线绷住（变）」。
  **情绪强度是表演幅度的依据**（写「生成全能提示词」时提示词里给了 EMOTION / EMOTION_INTENSITY；写分镜 JSON 时用你自己填的 emotion / emotion_intensity）：
  强度 1 = 克制到几乎只有眼神与呼吸的变化；强度 2 = 可见的面部与身体反应；
  强度 3 = 强情绪，**必须有外化的身体动作**（后退、踉跄、攥拳、发抖、猛然抬头、甩开手等）。**强度越高幅度越大，不许一律写成静立不动。**
  **台词镜必须写"听的人"的反应**：说话人每说完一句，对手要有可见反应（眼神、眉、呼吸、点头、缩肩、后仰、手指收紧）。
  只写说话人、把对手写成「闭口倾听 / 全程不出声 / 没有任何表情变化」= **不合格**。
- **台词期间镜头固定（对白镜硬规则）**：说话人开口的**这段时间里镜头保持固定** —— 不切镜、不推拉、不环绕、不升降（轻微自然手持可以）。
  为什么：口型与镜头运动叠加会把嘴糊掉，观众看不出在说话。
  同时：**只有当前说话人**可以有**发声口型**（张嘴说话、喉部发声动作）；其他角色**不得出现发声口型**，
  但**必须有反应性表演** —— 眼神、眉、呼吸、点头、缩肩、后仰、手指收紧照写，
  **禁止**把非说话人写成「闭口不动 / 全程不出声 / 面无表情」那种木头人（对手戏的表演一半在听的人身上）。台词必须与说话人的口型同步。
- **台词必须尽早出现（对白镜硬规则，实测踩过）**：有台词的镜头，台词就是**第一拍的内容** ——
  「[Shot 1]」里最多留 1 秒画面建立（起幅、人物入画），说话人随即开口。
  **禁止**把台词排到「第三秒起 / 第四秒起 / 第七秒起」这类时间点之后。实测：这类「先用 3-7 秒做动作或运镜、再开口」的写法，
  成片里声音真的要等到那个时间点才出来，前面几秒只有动作与音效，而画面里说话人的嘴早已在动（口型与声音错位，观感就是「嘴动了半天没声音」）。
  需要同时有动作或运镜时：写成「边说边做」（台词与动作同一拍），或把运镜挪到**台词说完之后**；唯一允许的前置是 ≤1 秒的固定起幅。
- **开口前不得有口型**：台词出现之前的画面里，说话人保持闭嘴、自然表情，不得出现张嘴、喉部起伏等说话动作；
  台词开始后口型与声音同步；台词说完之后，画面里任何人都不再做说话口型。
...
4. **台词合规**：台词原文逐字保留、包在 <d>…</d> 里，说话人编号正确，非说话人**不写发声口型但要有反应性表演**；
...
- 在 "storyboards" 数组的**每一个**镜头对象中必须有字符串字段 "narration"，且 **narration 一律不得为空字符串**。
- **剧本已有的旁白是唯一来源，必须逐字沿用**：若本集剧本里已经写了画外音/解说（通常标着「画外音：」「旁白：」），
  必须把这些句子**原字原句**分配到对应镜头 —— **严禁改写、缩写、润色、换说法，严禁另写新句子**：
  · 一幕剧本常含多句旁白：按句子顺序**一句一镜**地拆给各镜头；镜头数多于句子数时，多出的镜头沿用相邻句，不要自己编。
  · 镜头数少于句子数时，可把 1～3 句放进同一镜（只要该镜 duration 读得完），但文字必须与剧本**逐字一致**。
  · 只有剧本旁白完全覆盖不到的镜头，才允许自己补写，且必须与该镜画面一致、不得引入剧本没提到的技术点或结论。
- **shot_number 为 1 的第一个镜头**：必须有解说（直接使用剧本第一句），禁止留空。
...
**Output**: JSON with "storyboards" array. Each item: shot_number, segment_index, segment_title, title, shot_type, angle, time, location, scene_id, movement, lighting_style, depth_of_field, action, dialogue, narration, result, atmosphere, emotion, emotion_intensity, duration, bgm_prompt, sound_effect, characters (array of IDs), props (array of prop IDs), is_primary${uniExtra}. Return ONLY valid JSON, no markdown.`;
  ...
  const durationInstruction = durationHint
    ? `单镜时长**按内容动态决定**：**常规 8-15 秒**（下限 5 秒、上限 ${durationHint} 秒＝项目「每段最大秒数」）。硬性要求：① **含对白的镜头 duration ≥ 台词字数 ÷ 4.2 + 1 秒**（中文语速约 4.2 字/秒）；② 同一空间的连续动作**一镜到底**，需要更长时间就把它写长，而不是拆成好几条 5-6 秒的碎镜。`
...
function getPromoVideoSystemPrompt(cfg, segmentCount) {
  ...
  // 单幕时长（秒）与解说词字数同源，避免模板自相矛盾：
  // 原来格式里写 duration:10，却要求 150-200 字解说词 —— 175 字按中文 4.2 字/秒要念 41 秒，
  // 全片会被拖到三五分钟。这里统一按 duration 推算字数（10 秒 ≈ 42 字）。
  const segSeconds = 10;
  const segChars = Math.round(segSeconds * 4.2);
  const jsonNote = `\n\n**输出格式（必须严格遵守）**：\n返回一个 JSON 数组，包含 ${n} 个对象，每个对象格式如下：\n[\n  {\n    "segment": 1,\n    "title": "段落标题（5-10字）",\n    "narration": "本段解说词（约${segChars}字，画外音风格，正式大气）",\n    "visual": "画面描述（英文，适合AI图片生成，描述具体场景/画面/构图）",\n    "duration": ${segSeconds}\n  }\n]\n**必须只返回纯 JSON 数组，不要任何 markdown 代码块、说明文字。直接以 [ 开头，以 ] 结尾。**`;
  const _promoOverride = _overrideCache['promo_video_system'];
  const base = _promoOverride || `你是一位专业的企业宣传片策划。你的任务是根据用户提供的公司/产品信息，产出 ${n} 幕的宣传片分镜解说词大纲。

【第一优先级：忠实于输入】
用户给的如果本身就是现成的脚本、分镜表或文案，你的工作是**改写它**，不是另写一份：
1. **结构跟着输入走**：输入已经分了段（或标了时间轴），就按输入的**段数与顺序**输出，一段对一幕，不要重排叙事结构。输入没有分段时，再按「开场引入 → 技术/产品展示 → 核心优势 → 应用场景 → 愿景收尾」组织。
2. **输入里没有的，不要写**：不要新增技术点、参数、数字、品牌、场景、人物或结论，不要虚构效果与卖点。
3. **输入里的技术点、参数、数字、专有名词一个都不能丢、不能改**：型号、毫米数、毫瓦数、容量、时间这类量值必须原样出现。
4. **输入里已经写好的解说词/旁白，逐字沿用**，只允许为顺句做最小的连接；需要跨段合并时按原顺序拼接，不得改写措辞、不得换说法。
5. **画面描述要覆盖输入对应段落里的全部视觉要点**（主体、材质、光线、构图、镜头运动），不许压成一句概括。输入里若给了「AI 提示词」，按它的原意译成英文，不要增删要点。
6. 输入没写到的地方，宁可克制，也不要自己补。

【本片规格】
7. 不是剧本、不是故事，是宣传片分镜解说词大纲：每幕含解说词（中文、画外音风格、正式大气）+ 画面描述（英文，具体可执行、适合 AI 图片生成）。
8. 解说词写到**约 ${segSeconds} 秒能念完的量**（中文 4.2 字/秒，约 ${segChars} 字）即可。下面输出格式里的字数是字段示例，一律以本条为准 —— 按示例那么写，这一幕就要念 40 秒以上，全片会被拖长好几倍。
9. 不需要角色、对话、情节；画面可以是产品、器件、电路板、模组、产线、数据可视化。
10. 解说词避免空洞口号，只讲输入里有依据的技术点。`;
  return base + jsonNote;
}
...
/**
 * 全能模式（本地 MiniMax H3 Ref2VA，走 ComfyUI）：
