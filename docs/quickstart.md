...
在项目根目录提供了一键启动脚本，**同时启动后端和前端**（会自动清理残留端口，并在启动后校验两个服务是否真的可用）：

**Linux / macOS：**
```bash
./run_dev.sh            # 前台启动，Ctrl+C 一起停止
./run_dev.sh restart    # 重启
./run_dev.sh bg         # 后台启动，日志写 /tmp/localminidrama_*.log
./run_dev.sh stop       # 停止（前台与后台启动的都适用）
```
...
后端端口 5679、前端端口 3013。Windows 脚本会分别在两个窗口中启动服务并自动打开浏览器。
