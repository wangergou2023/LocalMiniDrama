...
function stripSpeechFromVideoPrompt(prompt) {
  let p = String(prompt || '')
  // 只去掉「画外音旁白」，**保留角色对白** —— 全能短剧里 <d>…</d> 就是角色的台词，
  // 整块删掉会让片子里没人说话（对白本来由 H3 直接念）。
  p = p.replace(/\(S\d+\)[^<]{0,80}?off-screen voice-over[^<]*?<d>[\s\S]*?<\/d>\s*。?/gi, '')
  p = p.replace(/(?:Voice-over narration|Narration)\s*[:：][^\n.]*\.?/gi, '')
  // 音效段里对旁白的音色/语气描述。要覆盖「旁白转为…」「旁白压低音量…」「旁白语速放缓…」
  // 这些写法（早期只匹配「旁白为/语气/音色/是」，漏了 6 个分镜，实测见 #276~#281）。
  p = p.split(/(?<=[。；])/).filter((x) => !/旁白/.test(x)).join('')
  // 经典字段里的整段旁白
  p = p.replace(/(?:解说旁白|旁白|画外音)\s*[：:][\s\S]*?(?=结果|景别|镜头角度|运镜|氛围|情绪|配乐|音效|时长|风格|$)/g, '')
  p = p
    .replace(/[；，]\s*([。；])/g, '$1')
